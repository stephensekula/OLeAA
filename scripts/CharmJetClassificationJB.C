...
void CharmJetClassificationJB(TString dir, TString input, TString filePattern = "*/out.root", TString taggers = "ip3dtagger")
{
  // Global options
  gStyle->SetOptStat(0);
  ...
  auto signal_train = default_data->CopyTree("jet.flavor==4", "", TMath::Floor(default_data->GetEntries() / 10.0));
  std::cout << "Signal Tree (Training): " << signal_train->GetEntries() << std::endl;

  auto background_train = default_data->CopyTree("(jet.flavor<4||jet.flavor==21)", "", TMath::Floor(default_data->GetEntries() / 10.0));
  std::cout << "Background Tree (Training): " << background_train->GetEntries() << std::endl;

  // auto u_background_train = default_data->CopyTree("(jet.flavor==2)", "",
  // TMath::Floor(default_data->GetEntries()/1.0));
  // std::cout << "u Background Tree (Training): " <<
  // u_background_train->GetEntries() << std::endl;

  // auto d_background_train = default_data->CopyTree("(jet.flavor==1)", "",
  // TMath::Floor(default_data->GetEntries()/1.0));
  // std::cout << "d Background Tree (Training): " <<
  // d_background_train->GetEntries() << std::endl;

  // auto g_background_train = default_data->CopyTree("(jet.flavor==21)", "",
  // TMath::Floor(default_data->GetEntries()/1.0));
  ...
  auto outputFile = TFile::Open(Form("CharmJetClassification_%s_Results.root",taggers.ReplaceAll(",","_").Data()), "RECREATE");
  ...
  for (Int_t itagger = 0; itagger < tagger_list->GetEntries(); itagger++) {
    ...
    // loader_ktagger.AddVariable("jet.PT",  "Jet p_{T}",          "GeV", 'F',
    // 0.0,
    // 1000.0);
    // loader_ktagger.AddVariable("jet.Eta", "Jet Pseudorapidity", "",    'F',
    // -5.0, 5.0);
    
    TCut charmjet_cut("jet.PT>5.0 && TMath::Abs(jet.Eta) < 3.0 && jet.flavor==4");
    TCut lightjet_cut("jet.PT>5.0 && TMath::Abs(jet.Eta) < 3.0 && (jet.flavor<4||jet.flavor==21)");
    ...
    if (tagger_name == "ktagger") {
    ...
    } else if (tagger_name == "jb_ip3dtagger") {

      loader.AddSpectator("TMath::Max(jet.PT,-1.0)", "jet.PT", "F");
      loader.AddSpectator("TMath::Max(jet.Eta,-10.0)", "jet.Eta", "F");
      loader.AddSpectator("TMath::Max(jet.Phi,-10.0)", "jet.Phi", "F");
      loader.AddSpectator("TMath::Max(jet.Mass,-1.0)", "jet.Mass", "F");
      loader.AddVariable("TMath::Max(jet.t1.PT,-1)", "jet.t1.PT", "F");
      loader.AddVariable("TMath::Max(jet.t1.sIP3D,-99)", "jet.t1.sIP3D", "F");
      loader.AddVariable("TMath::Max(jet.t2.PT,-1)", "jet.t2.PT", "F");
      loader.AddVariable("TMath::Max(jet.t2.sIP3D,-99)", "jet.t2.sIP3D", "F");
      loader.AddVariable("TMath::Max(jet.t3.PT,-1)", "jet.t3.PT", "F");
      loader.AddVariable("TMath::Max(jet.t3.sIP3D,-99)", "jet.t3.sIP3D", "F");
      loader.AddVariable("TMath::Max(jet.t4.PT,-1)", "jet.t4.PT", "F");
      loader.AddVariable("TMath::Max(jet.t4.sIP3D,-99)", "jet.t4.sIP3D", "F");
      ...
      factory.BookMethod(&loader, 
			 TMVA::Types::kMLP, "JBIP3DTagger",
			 "!H:!V:NeuronType=ReLU:VarTransform=Norm:NCycles=1000:HiddenLayers=N+16:TestRate=5:!UseRegulator");
    } else if (tagger_name == "jb_jet_tagger") {

      loader.AddVariable("TMath::Max(jet.PT,-1.0)", "jet.PT", "F");
      loader.AddVariable("TMath::Max(jet.Eta,-10.0)", "jet.Eta", "F");
      loader.AddVariable("TMath::Max(jet.Phi,-10.0)", "jet.Phi", "F");
      loader.AddVariable("TMath::Max(jet.Mass,-1.0)", "jet.Mass", "F");
      loader.AddVariable("TMath::Max(jet.t1.PT,-1)", "jet.t1.PT", "F");
      loader.AddVariable("TMath::Max(jet.t1.sIP3D,-99)", "jet.t1.sIP3D", "F");
      loader.AddVariable("TMath::Max(jet.t2.PT,-1)", "jet.t2.PT", "F");
      loader.AddVariable("TMath::Max(jet.t2.sIP3D,-99)", "jet.t2.sIP3D", "F");
      ...
      factory.BookMethod(&loader, 
			 ...
			 "!H:!V:NeuronType=ReLU:VarTransform=Norm:NCycles=1000:HiddenLayers=N+20:TestRate=5:!UseRegulator");
    } else if (tagger_name == "jb_subjet_tagger") {
      ...
      loader.AddVariable("TMath::Max(trimmedsubjet1.PT,-1.0)");
      loader.AddVariable("TMath::Max(trimmedsubjet1.Eta,-10.0)");
      // //loader.AddVariable("TMath::Max(trimmedsubjet1.Phi,-10.0)");
      loader.AddVariable("TMath::Max(trimmedsubjet1.Mass,-1.0)");
      loader.AddVariable("TMath::Max(trimmedsubjet1.t1.PT,-1.0)");
      loader.AddVariable("TMath::Max(trimmedsubjet1.t1.sIP3D,-99.0)");
      loader.AddVariable("TMath::Max(trimmedsubjet1.t2.PT,-1.0)");
      loader.AddVariable("TMath::Max(trimmedsubjet1.t2.sIP3D,-99.0)");
      // loader.AddVariable("TMath::Max(trimmedsubjet1.t3.PT,-1.0)");
      // loader.AddVariable("TMath::Max(trimmedsubjet1.t3.sIP3D,-99.0)");
      // //loader.AddVariable("TMath::Max(trimmedsubjet1.t4.PT,-1.0)");
      // //loader.AddVariable("TMath::Max(trimmedsubjet1.t4.sIP3D,-99.0)");

      loader.AddVariable("TMath::Max(trimmedsubjet2.PT,-1.0)");
      loader.AddVariable("TMath::Max(trimmedsubjet2.Eta,-10.0)");
      // //loader.AddVariable("TMath::Max(trimmedsubjet2.Phi,-10.0)");
      loader.AddVariable("TMath::Max(trimmedsubjet2.Mass,-1.0)");
      loader.AddVariable("TMath::Max(trimmedsubjet2.t1.PT,-1.0)");
      loader.AddVariable("TMath::Max(trimmedsubjet2.t1.sIP3D,-99.0)");
      loader.AddVariable("TMath::Max(trimmedsubjet2.t2.PT,-1.0)");
      loader.AddVariable("TMath::Max(trimmedsubjet2.t2.sIP3D,-99.0)");
      // //loader.AddVariable("TMath::Max(trimmedsubjet2.t3.PT,-1.0)");
      // //loader.AddVariable("TMath::Max(trimmedsubjet2.t3.sIP3D,-99.0)");
      // //loader.AddVariable("TMath::Max(trimmedsubjet2.t4.PT,-1.0)");
      // //loader.AddVariable("TMath::Max(trimmedsubjet2.t4.sIP3D,-99.0)");
      ...
      factory.BookMethod(&loader, 
			 TMVA::Types::kMLP, "JBSubjetTagger",
			 "!H:!V:NeuronType=ReLU:VarTransform=Norm:NCycles=1000:HiddenLayers=N+36:TestRate=5:!UseRegulator");
    }
