...
void CharmJetClassificationJB_Plots()
{
  // Global options
  gStyle->SetOptStat(0);
  ...
  std::vector<TString> input_folders = { "dataset_jb_ip3dtagger"};
  std::vector<TString> input_vars    = { "jet.t1.PT", "jet.t1.sIP3D", "jet.t2.PT", "jet.t2.sIP3D", "jet.t3.PT", "jet.t3.sIP3D", "jet.t4.PT", "jet.t4.sIP3D"};
  ...
  for (auto input_folder : input_folders) {
    ...
    for (auto input_var : input_vars) {
      ...
      testing_data.Add("CharmJetClassification_jb_ip3dtagger_Results.root");
      training_data.Add("CharmJetClassification_jb_ip3dtagger_Results.root");
      ...
      training_data.Project(h_signal->GetName(),     input_var.Data(), "jet.flavor==4");
      training_data.Project(h_background->GetName(), input_var.Data(), "jet.flavor<4 || jet.flavor==22");
      ...
      pad->SaveAs(Form("JB_%s.pdf", input_var.Data()));
