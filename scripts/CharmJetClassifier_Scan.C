...
void CharmJetClassifier_Scan(TString inputfile = "CharmJetClassification_Results.root",
  ...
  Float_t nlight = TestTree->GetEntries("jet_flavor<4 || jet_flavor==21");
  ...
  for (Float_t mva_cut = 0.000; mva_cut < 1.000; mva_cut += 0.001) {
    Float_t nlight_pass = TestTree->GetEntries(Form("jet_pt>5.0 && TMath::Abs(jet_eta) < 3.0 && met_et > 10.0 && (jet_flavor<4 || jet_flavor==21) && %s>%f", taggername.Data(), mva_cut));
  ...
  Float_t nlight_pass = TestTree->GetEntries(Form("jet_pt>5.0 && TMath::Abs(jet_eta) < 3.0 && met_et > 10.0 && (jet_flavor<4 || jet_flavor==21) && %s>%f", taggername.Data(), opt_cut));
  ...
  samples["Background"] = TCut("(jet_flavor<4 || jet_flavor==21)");
