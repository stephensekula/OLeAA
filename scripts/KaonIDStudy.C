                 ...
                 TString filePattern = "*/out.root")
{
  ...
  TCut                        cut_trueproton("TMath::Abs(pid_track_true_pid) == 2212");
  TCut                        cut_recoproton("TMath::Abs(pid_track_reco_pid) == 2212");
  ...
  Double_t ymax = 1.12;
  h_template->SetAxisRange(0.0, ymax, "Y");
  if (logplot) {
    ymax = 10.0;
    h_template->SetAxisRange(1.0e-5, ymax, "Y");
  }
  ...
  auto pi_misid_efficiency = new TEfficiency(*reco_pion_pt,
                                          *true_pion_pt);
  pi_misid_efficiency->SetMarkerColor(kRed);
  pi_misid_efficiency->SetFillColor(kRed);
  pi_misid_efficiency->SetLineColor(kRed);
  pi_misid_efficiency->SetMarkerStyle(kFullCrossX);

  // Proton -> Kaon ID
  auto true_proton_pt = GeneratePlot(draw_config, data, "True Proton PT", "pid_track_pt*TMath::CosH(pid_track_eta)", *cut_fiducial && cut_trueproton);
  auto reco_proton_pt = GeneratePlot(draw_config, data, "Reconstructed Proton PT", "pid_track_pt*TMath::CosH(pid_track_eta)", *cut_fiducial && cut_trueproton && cut_recokaon);


  auto proton_misid_efficiency = new TEfficiency(*reco_proton_pt,
                                          *true_proton_pt);
  proton_misid_efficiency->SetMarkerColor(kGray+2);
  proton_misid_efficiency->SetFillColor(kGray+2);
  proton_misid_efficiency->SetLineColor(kGray+2);
  proton_misid_efficiency->SetMarkerStyle(kFullDiamond);
  ...
  pi_misid_efficiency->Draw("E1 SAME");
  proton_misid_efficiency->Draw("E1 SAME");
  ...
  legend->AddEntry(pi_misid_efficiency, "#pi^{#pm} #rightarrow K^{#pm}", "lp");
  legend->AddEntry(proton_misid_efficiency, "p^{#pm} #rightarrow K^{#pm}", "lp");
  legend->Draw();
