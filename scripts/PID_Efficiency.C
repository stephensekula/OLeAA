...
void              PID_Efficiency(TString dir, TString input, TString trackname = "barrelDircTrack", TString filePattern = "*/out.root")
{
  ...
  if (trackname == "mRICHTrack") {
    ...
    etaMin       = -4.0;
    etaMax       = -1.0;
    cut_fiducial = new TCut(Form("%0.1f < %s.Eta && %s.Eta < %0.1f && %s.PT>0.1", etaMin, trackname.Data(), trackname.Data(), etaMax, trackname.Data()));
    xmax         = 12.0;
    ...
    etaMax       = 2.0;
    cut_fiducial = new TCut(Form("%0.1f < %s.Eta && %s.Eta < %0.1f && %s.PT>0.1", etaMin, trackname.Data(), trackname.Data(), etaMax, trackname.Data()));
  } else if (trackname == "dRICHagTrack") {
  ...
  TCut                        cut_truekaon(Form("((%s.PID & 0xffff0000) >> 16) == 321", trackname.Data()));
  TCut                        cut_recokaon(Form("(%s.PID & 0xffff) == 321", trackname.Data()));
  TCut                        cut_truepion(Form("((%s.PID & 0xffff0000) >> 16) == 211", trackname.Data()));
  TCut                        cut_recopion(Form("(%s.PID & 0xffff) == 211", trackname.Data()));
  ...
  // Kaon -> Kaon ID
  auto true_kaon_pt = GeneratePlot(draw_config, data, "True Kaon PT", Form("%s.PT*TMath::CosH(%s.Eta)", trackname.Data(), trackname.Data()), *cut_fiducial && cut_truekaon);
  auto reco_kaon_pt = GeneratePlot(draw_config, data, "Reconstructed Kaon PT", Form("%s.PT*TMath::CosH(%s.Eta)", trackname.Data(), trackname.Data()), *cut_fiducial && cut_truekaon && cut_recokaon);
  ...
  // Pion -> Kaon ID
  auto true_pion_pt = GeneratePlot(draw_config, data, "True Pion PT", Form("%s.PT*TMath::CosH(%s.Eta)", trackname.Data(), trackname.Data()), *cut_fiducial && cut_truepion);
  auto reco_pion_pt = GeneratePlot(draw_config, data, "Reconstructed Pion PT", Form("%s.PT*TMath::CosH(%s.Eta)", trackname.Data(), trackname.Data()), *cut_fiducial && cut_truepion && cut_recokaon);
  ...
  c1.SaveAs(Form("%s_PID_Efficiency.pdf", trackname.Data()));
}
